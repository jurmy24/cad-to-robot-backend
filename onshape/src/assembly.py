...
from onshape.api.client import Client
from onshape.src.robot import Joint
...
class Assembly:
    ...
    def __init__(self, config: Config):
        ...
        # Check if JSON file for the assembly already exists
        config_dir = os.path.dirname(self.config.config_file)
        assembly_file = os.path.join(config_dir, "assembly_data.json")
        features_file = os.path.join(config_dir, "features_data.json")
        matevalues_file = os.path.join(config_dir, "matevalues_data.json")
        if (
            os.path.exists(assembly_file)
            and os.path.exists(features_file)
            and os.path.exists(matevalues_file)
        ):
            print("Assembly data already exists, skipping retrieval from Onshape")
            self.update_assembly_data_from_json()
            self.find_instances()  # This uses self.assembly_data, so it can be called directly
            self.reload_features_from_json()
        else:
            print("Assembly data does not exist, retrieving from Onshape")
    ...
    def ensure_workspace_or_version(self):
        ...
        """
        if self.version_id:
            print((f"* Using configuration version ID {self.version_id} ..."))
        elif self.workspace_id:
            print((f"* Using configuration workspace ID {self.workspace_id} ..."))
        else:
            print(
    ...
    def find_assembly(self):
        ...
        if self.config.element_id:
            print((f"* Using configuration element ID {self.config.element_id} ..."))
    ...
    def retrieve_assembly(self):
        ...
        """
        print((f"* Retrieving assembly with id {self.element_id}"))
        ...
        # Save assembly data to JSON file
        import os
        import json
    ...
    def update_assembly_data_from_json(self):
        import os
        import json
        ...
        # Now we need to update the assembly data with the new configuration
        self.microversion_id = self.assembly_data["rootAssembly"][
            "documentMicroversion"
        ]
        self.occurrences = {}
        for occurrence in self.assembly_data["rootAssembly"]["occurrences"]:
    ...
    def load_features(self):
        ...
        with open(matevalues_file, "w") as f:
            json.dump(self.matevalues, f, indent=2)

    def reload_features_from_json(self):
        import os
        import json
    ...
    def process_mates(self):
        ...
        for data, occurrence_A, occurrence_B in self.feature_mating_two_occurrences():
            if data["name"].startswith("dof_"):
                ...
                print((f"+ Found DOF: {name} ({joint_type}) {limits_str}"))
        ...
        print((f"* Found total {len(self.dofs)} degrees of freedom"))
    ...
    def build_trees(self):
        ...
        print((f"* Found {len(self.root_nodes)} root nodes:"))
        for root_node in self.root_nodes:
            print((f"  - {self.body_instance(root_node)['name']}"))
    ...
    def read_parameter_value(self, parameter: str, name: str):
        ...
        if parameter["typeName"] == "BTMParameterConfigured":
            ...
            print((f"Coud not find the value for {name}"))
        else:
        ...
        for entry in self.matevalues["mateValues"]:
            if entry["mateName"] == name:
                if "rotationZ" in entry:
                    ...
                    print((f"Unknown offset type for {name}"))
        return None
    ...
    def get_limits(self, joint_type: str, name: str):
        ...
        if enabled:
            ...
            if joint_type != Joint.CONTINUOUS:
                print((f"WARNING: joint {name} of type {joint_type} has no limits"))
            return None
