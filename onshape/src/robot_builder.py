...
import json
import fnmatch
from onshape.src.geometry import Mesh
from onshape.src.assembly import Assembly
from onshape.src.config import Config
from onshape.src.robot import Part, Joint, Link, Robot, Relation, Closure
from onshape.src.csg import process as csg_process
...
class RobotBuilder:
    ...
    def add_part(self, occurrence: dict):
        ...
        if instance["partId"] == "":
            print((f"WARNING: Part '{instance['name']}' has no partId"))
            return

        part_name = instance["name"]
        extra = ""
        if instance["configuration"] != "default":
            extra = " (configuration: " + self.printable_configuration(instance) + ")"
        symbol = "+"
        if self.part_is_ignored(part_name, "visual") or self.part_is_ignored(
            ...
            if self.part_is_ignored(part_name, "visual"):
                extra += "(ignoring visual)"
            if self.part_is_ignored(part_name, "collision"):
                extra += " (ignoring collision)"

        print((f"{symbol} Adding part {part_name}{extra}"))
