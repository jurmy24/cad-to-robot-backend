...
class ProcessorConvexDecomposition(Processor):
    ...
    def check_coacd(self):
        if self.convex_decomposition:
            print(("* Checking CoACD presence..."))
            try:
                import coacd
                import trimesh
            except ImportError:
                print(("Can't import CoACD, disabling convex decomposition."))
                print(("TIP: consider installing CoACD:"))
                print(("pip install coacd trimesh"))
    ...
    def convex_decompose(self, part: Part):
        ...
        if len(collision_meshes) > 0:
            ...
            print((f"* Decomposed part {part.name} into {len(meshes)} convex shapes."))
