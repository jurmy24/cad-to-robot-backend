...
class ProcessorScad(Processor):
    ...
    def check_openscad(self):
        if self.use_scads:
            print(("* Checking OpenSCAD presence..."))
            try:
                subprocess.run(["openscad", "-v"])
            except FileNotFoundError:
                print(("Can't run openscad -v, disabling OpenSCAD support"))
                print(("TIP: consider installing openscad:"))
                print(("Linux:"))
                print(("sudo add-apt-repository ppa:openscad/releases"))
                print(("sudo apt-get update"))
                print(("sudo apt-get install openscad"))
                print(("Windows:"))
                print(("go to: https://openscad.org/downloads.html "))
                self.use_scads = False

    def process(self, robot: Robot):
        if self.use_scads:
            print(("+ Parsing OpenSCAD files..."))
