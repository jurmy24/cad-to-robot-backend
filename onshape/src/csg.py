...
def cube_parse(parameters, dilatation):
    ...
    return (extra + np.array(json.loads(results[0][0]), dtype=float) / 1000.0), results[
        0
    ][1] == "true"
        ...
        if line != "":
            if line[-1] == "{":
                ...
                if node == "cube":
                    ...
                    shapes.append(
                        {"type": "cube", "parameters": size, "transform": transform}
                    )
                if node == "cylinder":
                    ...
                    shapes.append(
                        {"type": "cylinder", "parameters": size, "transform": transform}
                    )
                if node == "sphere":
                    shapes.append(
                        {
